    ...
    */
    ...
    public void sort() {
        for (int i = 0; i < (n - 1); i++) {
            ...
            for (int j = (i + 1); j < n; j++){
                comp++;
                if (blank(series[menor].getCountry()).compareTo(blank(series[j].getCountry())) > 0){
                    menor = j;
                } else {
                    comp++;
                    if(blank(series[menor].getCountry()).compareTo(blank(series[j].getCountry())) == 0){
                        comp++;
                        if(blank(series[menor].getName()).compareTo(blank(series[j].getName())) > 0)
                            menor = j;
                    }
                }
            }
...
class TP03Q01{
    ...
    public static void saveFile(double time, int comp, int mov){
        try{
            FileWriter fileWriter = new FileWriter("724667_selecao.txt");
            BufferedWriter bw = new BufferedWriter(fileWriter); 
            bw.write(comp + "\t" + mov * 3 + "\t" + time + "s");
            bw.close();
        //Tratamento de exceções
        } catch(FileNotFoundException e) {
            System.out.println("Unable to open file '" + "724667_selecao.txt" + "'");                
        } catch(IOException e) {
            System.out.println("Error reading file '" + "724667_selecao.txt" + "'");
        }
    }    
