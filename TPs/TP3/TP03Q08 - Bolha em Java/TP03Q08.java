		...
		for (int i = (n - 1); i > 0; i--) {
			for (int j = 0; j < i; j++) {
				if (series[j].getSeasons() > series[j + 1].getSeasons()) {
                    swap(j, j+1);
				} else if(series[j].getSeasons() == series[j + 1].getSeasons() && series[j].getName().compareTo(series[j + 1].getName()) > 0){
                    swap(j, j+1);
                }
			}
		}
...
class TP03Q08{
    ...
    public static void saveFile(double time, int comp, int mov){
        try{
            FileWriter fileWriter = new FileWriter("724667_bolha.txt");
            BufferedWriter bw = new BufferedWriter(fileWriter); 
            bw.write(comp + "\t" + mov * 3 + "\t" + time + "s");
            bw.close();
        //Tratamento de exceções
        } catch(FileNotFoundException e) {
            System.out.println("Unable to open file '" + "724667_bolha.txt" + "'");                
        } catch(IOException e) {
            System.out.println("Error reading file '" + "724667_bolha.txt" + "'");
        }
    }    
