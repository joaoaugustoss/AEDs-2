    ...
    public void intercalar(int esq, int meio, int dir){
        ...
        for(i = j = 0, k = esq; k <= dir; k++){
            if(i == n1){
                ...
                series[k] = a1[i++];
            } else if(a1[i].getEpisodes() < a2[j].getEpisodes()){
                series[k] = a1[i++];
            } else if(a1[i].getEpisodes() == a2[j].getEpisodes()){
...
class TP03Q09{
    ...
    public static void saveFile(double time, int comp, int mov){
        try{
            FileWriter fileWriter = new FileWriter("724667_mergesort.txt");
            BufferedWriter bw = new BufferedWriter(fileWriter); 
            bw.write(comp + "\t" + mov * 3 + "\t" + time + "s");
            bw.close();
        //Tratamento de exceções
        } catch(FileNotFoundException e) {
            System.out.println("Unable to open file '" + "724667_mergesort.txt" + "'");                
        } catch(IOException e) {
            System.out.println("Error reading file '" + "724667_mergesort.txt" + "'");
        }
    }    
