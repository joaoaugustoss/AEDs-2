    ...
    */
    ...
    public static void sort() {
		for (int i = 1; i < n; i++) {
            ...
            while ((j >= 0) && (series[j].getLanguage().compareTo(tmp.getLanguage()) == 0) && (series[j].getName().compareTo(tmp.getName()) > 0)) {
                comp+=3;
                mov++;
                series[j + 1] = series[j];
                j--;
...
class TP03Q03{
    ...
    public static void saveFile(double time, int comp, int mov){
        try{
            FileWriter fileWriter = new FileWriter("724667_insercao.txt");
            BufferedWriter bw = new BufferedWriter(fileWriter); 
            bw.write(comp + "\t" + mov * 3 + "\t" + time + "s");
            bw.close();
        //Tratamento de exceções
        } catch(FileNotFoundException e) {
            System.out.println("Unable to open file '" + "724667_insercao.txt" + "'");                
        } catch(IOException e) {
            System.out.println("Error reading file '" + "724667_insercao.txt" + "'");
        }
    }    
