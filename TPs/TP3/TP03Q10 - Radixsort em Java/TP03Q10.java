...
class Lista {
    ...
    */
    ...
    static int getMaior(){
		...
		for (int i = 1; i < n; i++){
            comp++;
			if ((series[i].getEpisodes() * 1000 + series[i].getSeasons()) > maior)
				maior = (series[i].getEpisodes() * 1000 + series[i].getSeasons());
        }
		return maior;
	}
	...
	private static void countingsort(int exp) {
        ...
        for (int i = 0; i < n; count[((series[i].getEpisodes() * 1000 + series[i].getSeasons()) / exp) % 10]++, i++);

        //Agora, o count[i] contem o numero de elemento menores ou iguais a i
        for(int i = 1; i < tamCount; count[i] += count[i-1], i++);

        //Ordenando
        for(int i = n-1; i >= 0 ; ordenado[count[((series[i].getEpisodes() * 1000 + series[i].getSeasons()) / exp) % 10]-1] = series[i], count[((series[i].getEpisodes() * 1000 + series[i].getSeasons()) / exp) % 10]--, i--);
    ...
    public void bolha() {
		for (int i = (n - 1); i > 0; i--) {
			for (int j = 0; j < i; j++) {
                if((series[j].getEpisodes() * 1000 + series[j].getSeasons()) == (series[j+1].getEpisodes() * 1000 + series[j+1].getSeasons()) && series[j].getName().compareTo(series[j + 1].getName()) > 0){
                    swap(j, j+1);
                }
			}
		}
...
class TP03Q10{
    ...
    public static void saveFile(double time, int comp, int mov){
        try{
            FileWriter fileWriter = new FileWriter("724667_radixsort.txt");
            BufferedWriter bw = new BufferedWriter(fileWriter); 
            bw.write(comp + "\t" + mov * 3 + "\t" + time + "s");
            bw.close();
        //Tratamento de exceções
        } catch(FileNotFoundException e) {
            System.out.println("Unable to open file '" + "724667_radixsort.txt" + "'");                
        } catch(IOException e) {
            System.out.println("Error reading file '" + "724667_radixsort.txt" + "'");
        }
    }    
