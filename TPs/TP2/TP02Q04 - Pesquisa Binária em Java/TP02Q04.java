...
public class TP02Q04{
    ...
    public static void main(String[] args){
        ...
        long fim = now();
        saveFile((fim - inicio)/1000.0);
    }
    //método para salvar o tempo de execução e o número de repetições no arquivo .txt
    public static void saveFile(double time){
        try{
            FileWriter fileWriter = new FileWriter("724667_binária.txt");
            BufferedWriter bw = new BufferedWriter(fileWriter); 
            bw.write("Tempo de execução: " + time + "s");
            bw.newLine();
            bw.write("Número de comparações: " + count);
            bw.close();
        //Tratamento de exceções
        } catch(FileNotFoundException e) {
            System.out.println("Unable to open file '" + "724667_binária.txt" + "'");                
        } catch(IOException e) {
            System.out.println("Error reading file '" + "724667_binária.txt" + "'");
        }
    }
    ...
    //método para relizar a pesquisa binária dentro do vetor de séries
    public static boolean pesqBin(Serie[] serie, String entrada){
        boolean resp = false;
        int dir = (serie.length - 1), esq = 0, meio;

        while (esq <= dir){
            count++;
            meio = (esq + dir) / 2;
            if(entrada.equals(serie[meio].getName())){
                resp = true;
                esq = dir + 1;
            } else if (entrada.compareTo(serie[meio].getName()) > 0){
                esq = meio + 1;
            } else {
                dir = meio - 1;
            }
    ...
    //método de inserção para ordenar o vetor de strings
